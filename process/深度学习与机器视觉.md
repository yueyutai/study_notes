...
这会导致下一层输入总是偏正，使梯度更新方向受限制，优化效率降低。
...
梯度不会像 sigmoid/tanh 那样在正区间饱和。

---

#### 3）收敛速度快

ReLU 在深度神经网络中通常比 sigmoid/tanh 更容易训练。

CS231n 的课程笔记也将 ReLU 与 He 初始化作为实践推荐组合。([CS231n](https://cs231n.github.io/neural-networks-2/ "CS231n Deep Learning for Computer Vision"))
...
```text
如果没有激活函数，多层神经网络中的线性变换可以合并成一个线性变换，模型本质上仍然是线性分类器，无法表达复杂的非线性关系。激活函数的作用是引入非线性，使网络能够学习复杂决策边界。常见激活函数包括 sigmoid、tanh、ReLU、Leaky ReLU 等，其中 ReLU 因为计算简单、缓解梯度消失、收敛速度快，在 CNN 中非常常用。
```

---

### 问题 2：为什么 ReLU 比 sigmoid 更常用？

可以回答：

```text
Sigmoid 在输入较大或较小时容易进入饱和区，导数接近 0，导致梯度消失；同时 sigmoid 输出不是零中心，会影响优化效率。ReLU 在正半轴梯度恒为 1，计算简单，收敛速度快，因此在 CNN 和深度网络中更常用。不过 ReLU 也可能出现 Dead ReLU，因此可以用 Leaky ReLU、PReLU 等改进版本缓解。
```

---

### 问题 3：为什么不能把权重全部初始化为 0？

可以回答：

```text
如果同一层神经元权重都初始化为 0，那么它们的输出相同，反向传播得到的梯度也相同，后续更新仍然相同。这样多个神经元无法学习不同特征，导致对称性无法打破。因此神经网络通常使用随机初始化，例如 Xavier 初始化或针对 ReLU 的 He 初始化。
```

---

### 问题 4：Batch Normalization 的作用是什么？

可以回答：

```text
Batch Normalization 通过对 mini-batch 内的激活值进行标准化，使每一层输入分布更加稳定。它可以加快收敛、允许使用更大学习率、降低网络对初始化的敏感性，并具有一定正则化效果。通常 BN 放在线性层或卷积层之后、激活函数之前，并通过可学习参数 gamma 和 beta 保留网络表达能力。
```

---

### 问题 5：训练时 loss 不下降怎么办？

可以回答：

```text
首先检查数据和标签是否正确，然后看学习率是否合适。如果学习率太大，loss 可能震荡甚至变成 NaN；如果太小，loss 会下降很慢。还要检查初始化、激活函数、数据预处理、正则化强度和反向传播实现。如果是自己写代码，还应该做梯度检查。
```
...
```text
更深的网络更难训练，需要合适初始化、归一化、残差连接和正则化。
```
...
```text
BN 是网络中的可微模块，还包含可学习参数 gamma 和 beta，并且训练/测试阶段使用的统计量不同。
```
...
> 第六讲的核心是：**训练神经网络不仅是写出网络结构，还要通过合理的激活函数、数据预处理、权重初始化、Batch Normalization、正则化和超参数搜索，让优化过程稳定、可控并具备泛化能力。**