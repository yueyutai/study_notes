...
计算图把复杂函数拆成简单操作节点：
![](assets/深度学习与机器视觉/file-20260605221742475.png)
每个节点负责：
...
***
我无法直接读取该 B 站视频的完整字幕，所以不能保证逐字对应视频讲解。下面是按“第五讲 CNN”的核心技术内容整理的 **Markdown 笔记版**，重点抓概念、公式、结构和面试可用表达。参考了 Stanford CS231n Lecture 5 中关于 CNN 的定义、卷积层、padding/stride、pooling 等内容：其课程指出，普通两层神经网络会破坏图像的空间结构，而 CNN 通过卷积层保留空间结构并学习局部特征。
